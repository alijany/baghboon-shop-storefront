    ...
    <div className="flex flex-col flex-1">
      ...
      <div className="space-y-6 flex-1 flex flex-col justify-between p-6">
        {process.env.FEATURE_SEARCH_ENABLED && (
          ...
          >
            <MagnifyingGlassMini />
            <span placeholder="محصولات جستجو کنید" className="text-base-regular">
              محصولات جستجو کنید
            </span>
          </button>
        )}
            ...
            <li className="bg-gray-50 p-4 rounded-rounded">
              <Link href="/store">
                <button
                  ...
                  onClick={close}
                >
                  <span className="sr-only">به فروشگاه بروید</span>
                  <span>فروشگاه</span>
                    ...
                    <Link href={`/collections/${collection.handle}`}>
                      ...
                      >
                        <span className="sr-only">
                          برو به مجموعه ی {collection.title}
                        </span>
          ...
          <div className="flex flex-col gap-y-8 text-small-regular">
            {!customer ? (
              <div className="flex flex-col gap-y-4 ">
                <span className="text-gray-700 uppercase">حساب</span>
                <Link href={`/account/login`} passHref>
                  <button
                    className="flex items-center justify-between border-b border-gray-200 py-2 w-full"
                    onClick={close}
                  >
                    <span className="sr-only">به ورود به سیستم بروید</span>
                    <span className="normal-case">ورود</span>
                    <ChevronDown className="-rotate-90" />
                  </button>
                </Link>
              </div>
              ...
              <div className="flex flex-col gap-y-4">
                <span className="text-gray-700 uppercase">وارد شدید به عنوان</span>
                <Link href={`/account`} passHref>
                  <button
                    className="flex items-center justify-between border-b border-gray-200 py-2 w-full"
                    onClick={close}
                  >
                    <span className="sr-only">به صفحه حساب کاربری بروید</span>
            ...
            <div className="flex flex-col gap-y-4">
              <span className="text-gray-700 uppercase">تحویل</span>
              <button
                className="flex items-center justify-between border-b border-gray-200 py-2"
                onClick={setScreenCountry}
              >
                <span className="sr-only">
                  برای انتخاب منطقه ارسال کلیک کنید
                </span>
